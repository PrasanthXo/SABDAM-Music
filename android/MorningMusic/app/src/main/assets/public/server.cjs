...
async function fetchTrendingSongsFromGemini() {
  const verifiedHits = [
...
// server/youtubeConfig.ts
var YouTubeConfigManager = class _YouTubeConfigManager {
  constructor() {
  }
  static getInstance() {
    if (!_YouTubeConfigManager.instance) {
      _YouTubeConfigManager.instance = new _YouTubeConfigManager();
    }
    return _YouTubeConfigManager.instance;
  }
  /**
...
var youtubeConfig = YouTubeConfigManager.getInstance();
...
function normalizeSmtpConfig() {
  ...
  const rawPort = process.env.SMTP_PORT ? Number(process.env.SMTP_PORT) : void 0;
  if (!rawUser || !rawPass) {
    return null;
  }
...
function getTransporter() {
  ...
  if (config) {
    try {
      cachedTransporter = import_nodemailer.default.createTransport(config);
...
async function sendViaResend(apiKey, toEmail, subject, htmlContent, textContent) {
...
async function sendOtpEmail(toEmail, code, purpose = "signin", recipientName) {
...
<body style="margin:0;padding:0;background-color:#09090b;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#f4f4f5;">
  ...
  if (process.env.RESEND_API_KEY && process.env.RESEND_API_KEY.startsWith("re_")) {
    ...
    if (res.success) {
      console.log(`\u{1F4E8} [Email Delivered via Resend] To: ${toEmail} | Code: [${code}] | ID: ${res.messageId}`);
      return res;
    }
  }
  ...
  if (transporter) {
    try {
      const fromAddr = process.env.SMTP_FROM || process.env.EMAIL_FROM || process.env.GMAIL_USER || process.env.EMAIL_USER || process.env.SMTP_USER || `SABDHAM Music <noreply@sabdham.com>`;
      ...
      console.log(`\u{1F4E8} [Email Delivered via SMTP] To: ${toEmail} | MessageId: ${info.messageId}`);
  ...
  console.log(`
======================================================`);
  console.log(`\u{1F4E8} [EMAIL DISPATCH TO INBOX]`);
...
var firebase_applet_config_default = {
  projectId: "extreme-azimuth-pmn89",
  appId: "1:88609021288:web:a7d1ec4e977410c3320add",
  apiKey: "AIzaSyBv7WdgtX25TvQlLhLT4xo2h8vz5wBy2p8",
  authDomain: "extreme-azimuth-pmn89.firebaseapp.com",
  firestoreDatabaseId: "ai-studio-morningmusic-18ec9830-f13e-4491-9027-3735bf32c892",
  storageBucket: "extreme-azimuth-pmn89.firebasestorage.app",
  messagingSenderId: "88609021288",
  measurementId: "",
  oAuthClientId: "88609021288-b9q097m0edve9f55u82s72unjk7l05e6.apps.googleusercontent.com",
  recaptchaSiteKey: ""
};
...
function getAdminAuth() {
  ...
  try {
    ...
    adminAuthInstance = (0, import_auth.getAuth)(app2);
    return adminAuthInstance;
  } catch (err) {
...
__export(schema_exports, {
  ...
  playlistTracksRelations: () => playlistTracksRelations,
  userCustomSongs: () => userCustomSongs,
  userCustomSongsRelations: () => userCustomSongsRelations,
  userFavorites: () => userFavorites,
  userFavoritesRelations: () => userFavoritesRelations,
  userPlaylists: () => userPlaylists,
  userPlaylistsRelations: () => userPlaylistsRelations,
  userRecentlyPlayed: () => userRecentlyPlayed,
  userRecentlyPlayedRelations: () => userRecentlyPlayedRelations,
...
var users = (0, import_pg_core.pgTable)("users", {
  ...
  // Firebase Auth UID or Custom User ID
...
var userFavorites = (0, import_pg_core.pgTable)("user_favorites", {
  id: (0, import_pg_core.serial)("id").primaryKey(),
  userUid: (0, import_pg_core.text)("user_uid").references(() => users.uid, { onDelete: "cascade" }).notNull(),
  trackId: (0, import_pg_core.text)("track_id").notNull(),
  trackData: (0, import_pg_core.text)("track_data"),
  // JSON serialized Track metadata
  createdAt: (0, import_pg_core.timestamp)("created_at").defaultNow()
});
var userPlaylists = (0, import_pg_core.pgTable)("user_playlists", {
  id: (0, import_pg_core.text)("id").primaryKey(),
  // Custom Playlist ID (e.g., pl_1710000000000_xyz)
  userUid: (0, import_pg_core.text)("user_uid").references(() => users.uid, { onDelete: "cascade" }).notNull(),
...
var playlistTracks = (0, import_pg_core.pgTable)("playlist_tracks", {
  id: (0, import_pg_core.serial)("id").primaryKey(),
  playlistId: (0, import_pg_core.text)("playlist_id").references(() => userPlaylists.id, { onDelete: "cascade" }).notNull(),
  trackId: (0, import_pg_core.text)("track_id").notNull(),
  trackData: (0, import_pg_core.text)("track_data"),
  // JSON serialized Track metadata
  position: (0, import_pg_core.integer)("position").default(0),
  addedAt: (0, import_pg_core.timestamp)("added_at").defaultNow()
});
var userCustomSongs = (0, import_pg_core.pgTable)("user_custom_songs", {
  id: (0, import_pg_core.text)("id").primaryKey(),
  // Song ID (e.g., cs_... or custom id)
  userUid: (0, import_pg_core.text)("user_uid").references(() => users.uid, { onDelete: "cascade" }).notNull(),
  title: (0, import_pg_core.text)("title").notNull(),
  artist: (0, import_pg_core.text)("artist").notNull(),
  audioUrl: (0, import_pg_core.text)("audio_url").notNull(),
  artwork: (0, import_pg_core.text)("artwork"),
  duration: (0, import_pg_core.integer)("duration").default(0),
  youtubeId: (0, import_pg_core.text)("youtube_id"),
  lyrics: (0, import_pg_core.text)("lyrics"),
  createdAt: (0, import_pg_core.timestamp)("created_at").defaultNow()
});
var userRecentlyPlayed = (0, import_pg_core.pgTable)("user_recently_played", {
  id: (0, import_pg_core.serial)("id").primaryKey(),
  userUid: (0, import_pg_core.text)("user_uid").references(() => users.uid, { onDelete: "cascade" }).notNull(),
  trackId: (0, import_pg_core.text)("track_id").notNull(),
  trackData: (0, import_pg_core.text)("track_data"),
  // JSON serialized Track metadata
  playedAt: (0, import_pg_core.timestamp)("played_at").defaultNow()
});
var userSettings = (0, import_pg_core.pgTable)("user_settings", {
  id: (0, import_pg_core.serial)("id").primaryKey(),
  userUid: (0, import_pg_core.text)("user_uid").references(() => users.uid, { onDelete: "cascade" }).notNull().unique(),
  audioQuality: (0, import_pg_core.text)("audio_quality").default("high"),
  crossfade: (0, import_pg_core.boolean)("crossfade").default(true),
  equalizerPreset: (0, import_pg_core.text)("equalizer_preset").default("Dynamic Clarity"),
  offlineMode: (0, import_pg_core.boolean)("offline_mode").default(false),
  updatedAt: (0, import_pg_core.timestamp)("updated_at").defaultNow()
});
...
var userFavoritesRelations = (0, import_drizzle_orm.relations)(userFavorites, ({ one }) => ({
  user: one(users, {
    fields: [userFavorites.userUid],
    references: [users.uid]
  })
}));
var userPlaylistsRelations = (0, import_drizzle_orm.relations)(userPlaylists, ({ one, many }) => ({
  user: one(users, {
...
var playlistTracksRelations = (0, import_drizzle_orm.relations)(playlistTracks, ({ one }) => ({
  playlist: one(userPlaylists, {
    fields: [playlistTracks.playlistId],
    references: [userPlaylists.id]
  })
}));
var userCustomSongsRelations = (0, import_drizzle_orm.relations)(userCustomSongs, ({ one }) => ({
  user: one(users, {
    fields: [userCustomSongs.userUid],
    references: [users.uid]
  })
}));
var userRecentlyPlayedRelations = (0, import_drizzle_orm.relations)(userRecentlyPlayed, ({ one }) => ({
  user: one(users, {
...
async function getUserLibraryFromDb(uid) {
  try {
    const favRecords = await db.select().from(userFavorites).where((0, import_drizzle_orm2.eq)(userFavorites.userUid, uid));
    const likedTrackIds = favRecords.map((r) => r.trackId);
    const playlistRecords = await db.select().from(userPlaylists).where((0, import_drizzle_orm2.eq)(userPlaylists.userUid, uid));
    let customPlaylists = [];
    if (playlistRecords.length > 0) {
      const playlistIds = playlistRecords.map((p) => p.id);
      const trackRecords = await db.select().from(playlistTracks).where((0, import_drizzle_orm2.inArray)(playlistTracks.playlistId, playlistIds));
      const tracksByPlaylist = /* @__PURE__ */ new Map();
      ...
      customPlaylists = playlistRecords.map((pl) => {
        ...
        return {
          ...
          tracks: group.tracks,
          createdAt: pl.createdAt ? pl.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
          updatedAt: pl.updatedAt ? pl.updatedAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
          userId: pl.userUid
        };
      });
    }
    const songRecords = await db.select().from(userCustomSongs).where((0, import_drizzle_orm2.eq)(userCustomSongs.userUid, uid));
    const customSongs = songRecords.map((s) => ({
      id: s.id,
      title: s.title,
      artist: s.artist,
      audioUrl: s.audioUrl,
      ...
      youtubeId: s.youtubeId || void 0,
      lyrics: s.lyrics || void 0,
      createdAt: s.createdAt ? s.createdAt.toISOString() : (/* @__PURE__ */ new Date()).toISOString()
    }));
    const recentsRecords = await db.select().from(userRecentlyPlayed).where((0, import_drizzle_orm2.eq)(userRecentlyPlayed.userUid, uid)).orderBy((0, import_drizzle_orm2.desc)(userRecentlyPlayed.playedAt)).limit(50);
    const recentlyPlayed = recentsRecords.map((r) => {
      if (r.trackData) {
        try {
    ...
    const settingsRecord = await db.select().from(userSettings).where((0, import_drizzle_orm2.eq)(userSettings.userUid, uid)).limit(1);
...
async function saveUserLibraryToDb(uid, data) {
  try {
    if (Array.isArray(data.likedTrackIds)) {
      await db.delete(userFavorites).where((0, import_drizzle_orm2.eq)(userFavorites.userUid, uid));
    ...
    if (Array.isArray(data.customPlaylists)) {
      const playlistMap = /* @__PURE__ */ new Map();
      ...
      if (currentPlaylistIds.length > 0) {
        await db.delete(userPlaylists).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(userPlaylists.userUid, uid), (0, import_drizzle_orm2.notInArray)(userPlaylists.id, currentPlaylistIds)));
      } else {
        await db.delete(userPlaylists).where((0, import_drizzle_orm2.eq)(userPlaylists.userUid, uid));
      }
      for (const pl of uniquePlaylists) {
        await db.insert(userPlaylists).values({
          ...
          coverUrl: pl.coverUrl || "",
          isCustom: pl.isCustom ?? true,
          createdAt: pl.createdAt ? new Date(pl.createdAt) : /* @__PURE__ */ new Date(),
          updatedAt: /* @__PURE__ */ new Date()
        }).onConflictDoUpdate({
        ...
        await db.delete(playlistTracks).where((0, import_drizzle_orm2.eq)(playlistTracks.playlistId, pl.id));
    ...
    if (Array.isArray(data.customSongs)) {
      ...
      if (currentSongIds.length > 0) {
        await db.delete(userCustomSongs).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(userCustomSongs.userUid, uid), (0, import_drizzle_orm2.notInArray)(userCustomSongs.id, currentSongIds)));
      } else {
        await db.delete(userCustomSongs).where((0, import_drizzle_orm2.eq)(userCustomSongs.userUid, uid));
      }
      for (const s of uniqueSongs) {
        await db.insert(userCustomSongs).values({
          id: s.id,
          userUid: uid,
          title: s.title,
    ...
    if (Array.isArray(data.recentlyPlayed)) {
      await db.delete(userRecentlyPlayed).where((0, import_drizzle_orm2.eq)(userRecentlyPlayed.userUid, uid));
...
async function getUserFavorites(uid) {
  try {
    const records = await db.select().from(userFavorites).where((0, import_drizzle_orm2.eq)(userFavorites.userUid, uid));
...
async function addUserFavorite(uid, trackId, trackData) {
  try {
    await db.insert(userFavorites).values({
      userUid: uid,
      trackId,
...
async function removeUserFavorite(uid, trackId) {
  try {
    await db.delete(userFavorites).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(userFavorites.userUid, uid), (0, import_drizzle_orm2.eq)(userFavorites.trackId, trackId)));
    return { success: true };
  } catch (error) {
...
async function deleteUserPlaylist(uid, playlistId) {
  try {
    await db.delete(playlistTracks).where((0, import_drizzle_orm2.eq)(playlistTracks.playlistId, playlistId));
    await db.delete(userPlaylists).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(userPlaylists.userUid, uid), (0, import_drizzle_orm2.eq)(userPlaylists.id, playlistId)));
    return true;
  } catch (error) {
...
app.use((req, res, next) => {
  ...
  const normalized = import_path.default.posix.normalize(cleanPath);
  if (normalized === "/data" || normalized.startsWith("/data/") || normalized.startsWith("/.env") || normalized.startsWith("/server.ts") || normalized.startsWith("/..") || normalized.includes("/../")) {
...
async function robustFetchJson(url, options = {}) {
  const headers = {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`API returned error ${res.status} (non-JSON): ${text2.substring(0, 100)}`);
    }
    throw new Error(`API returned invalid JSON: ${text2.substring(0, 100)}`);
  }
  if (!res.ok) {
...
var userDataStore = /* @__PURE__ */ new Map();
var sessions = /* @__PURE__ */ new Map();
var resetCodes = /* @__PURE__ */ new Map();
var otpStore = /* @__PURE__ */ new Map();
var DATA_DIR = import_path.default.join(process.cwd(), "data");
var DATA_FILE = import_path.default.join(DATA_DIR, "users.json");
    ...
    if (import_fs.default.existsSync(DATA_FILE)) {
      ...
      if (parsed.users && Array.isArray(parsed.users)) {
        parsed.users.forEach((u) => {
          users2.set(u.email.toLowerCase(), u);
      ...
      if (parsed.sessions && Array.isArray(parsed.sessions)) {
        const now = Date.now();
        parsed.sessions.forEach(([tok, sess]) => {
...
function savePersistedData() {
  try {
    ...
    const payload = {
      users: Array.from(users2.values()),
...
function setAuthCookie(res, token, maxAgeDays = 7) {
...
async function getAuthUser(req) {
  ...
  try {
    ...
    if (decodedToken && decodedToken.uid) {
      ...
      let user = usersById.get(decodedToken.uid) || (email ? users2.get(email) : null);
      if (!user && email) {
        user = {
          id: decodedToken.uid,
          email,
...
function initUserData(userId, email) {
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const existing = users2.get(email);
    if (existing) {
    ...
    const passwordHash = await import_bcryptjs.default.hash(finalPassword, saltRounds);
    const id = `usr_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 8)}`;
    const displayName = rawName && typeof rawName === "string" && rawName.trim().length > 0 ? rawName.trim().slice(0, 50) : email.split("@")[0].charAt(0).toUpperCase() + email.split("@")[0].slice(1);
    ...
    initUserData(id);
    savePersistedData();
    getOrCreateUser(newUser.id, newUser.email, newUser.name, void 0).catch(() => {
    });
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    setAuthCookie(res, token, 7);
    console.log(`
\u{1F511} [Auth] User signed in: ${email} (${user.name})`);
    return res.json({
      success: true,
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    console.log(`
\u{1F510} [Auth] Password Reset Code generated for ${email}: [REDACTED]`);
...
app.post("/api/auth/reset-password", async (req, res) => {
  try {
    ...
    setAuthCookie(res, token, 7);
    console.log(`
\u{1F389} [Auth] Password reset successfully for ${email}`);
    return res.json({
      success: true,
...
app.post("/api/auth/otp/send", async (req, res) => {
  try {
    ...
    otpStore.set(email, {
      ...
      expiresAt,
      purpose,
      name: rawName && typeof rawName === "string" ? rawName.trim() : void 0,
      attempts: 0
    });
    console.log(`
\u{1F4E8} [Auth Email OTP] Code generated for ${email}: [REDACTED] (purpose: ${purpose}, existing: ${!!existing})`);
    const mailResult = await sendOtpEmail(email, code, purpose, rawName);
...
app.post("/api/auth/otp/verify", async (req, res) => {
  try {
    ...
    otpStore.delete(email);
    let user = users2.get(email);
    let isNewUser = false;
    if (!user) {
      isNewUser = true;
      const id = `usr_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 8)}`;
      const assignedName = rawName && typeof rawName === "string" && rawName.trim() || record.name && record.name.trim() || email.split("@")[0];
      ...
      savePersistedData();
      try {
        await getOrCreateUser(user.id, user.email, user.name, void 0);
      } catch (dbErr) {
    ...
    setAuthCookie(res, token, 7);
    console.log(`
\u{1F389} [Auth OTP] Successful authentication: ${user.email} (${user.name}) [isNewUser: ${isNewUser}]`);
    return res.json({
      success: true,
      message: isNewUser ? `Account created successfully! Welcome to SABDHAM, ${user.name}.` : `Welcome back, ${user.name}! Signed in successfully.`,
      token,
      user: sanitizeUser(user),
      isNewUser
    });
...
app.post("/api/auth/request-data-deletion", (req, res) => {
  try {
    ...
    const existing = users2.get(cleanEmail);
    if (existing) {
      users2.delete(cleanEmail);
      if (existing.id) {
...
app.post("/api/auth/google", requireAuth, async (req, res) => {
  try {
    ...
    let localUser = users2.get(req.user.email);
    if (!localUser) {
      localUser = {
        id: req.user.uid,
      ...
      users2.set(req.user.email, localUser);
      usersById.set(req.user.uid, localUser);
      initUserData(req.user.uid);
      savePersistedData();
    } else {
      localUser.lastLoginAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.get("/api/db/favorites", requireAuth, async (req, res) => {
  try {
    if (!req.user?.uid) {
...
app.post("/api/db/favorites", requireAuth, async (req, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
...
app.delete("/api/db/favorites/:trackId", requireAuth, async (req, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
...
app.delete("/api/db/playlists/:playlistId", requireAuth, async (req, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    if (current && current.customPlaylists) {
      current.customPlaylists = current.customPlaylists.filter((p) => p.id !== playlistId);
...
app.get("/api/user/data", async (req, res) => {
  ...
  try {
    ...
    if (dbLibrary && (dbLibrary.customPlaylists.length > 0 || dbLibrary.likedTrackIds.length > 0 || dbLibrary.customSongs.length > 0 || dbLibrary.recentlyPlayed.length > 0)) {
      userDataStore.set(user.id, dbLibrary);
      if (user.email) {
        userDataStore.set(user.email.toLowerCase(), dbLibrary);
      }
      savePersistedData();
  ...
  try {
    getOrCreateUser(user.id, user.email, user.name, user.avatarUrl).then(() => {
      saveUserLibraryToDb(user.id, data);
    }).catch(() => {
    });
  } catch {
  }
...
async function fetchPublicSpotifyPlaylistEmbed(playlistId) {
...
app.get("/api/spotify/playlist-tracks", async (req, res) => {
  ...
  try {
    ...
    if (spotifyToken && spotifyToken.startsWith("Bearer ")) {
      try {
        let allItems = [];
        let nextUrl = `https://api.spotify.com/v1/playlists/${cleanId}/tracks?limit=100`;
        try {
    ...
    const trackList = (entity.trackList || []).map((t) => ({
      track: {
        id: t.uid || t.uri,
        name: t.title,
...
function getClientDb() {
  if (!_clientDb) {
    try {
      const clientApp = (0, import_app2.initializeApp)(firebase_applet_config_default);
      _clientDb = firebase_applet_config_default.firestoreDatabaseId ? (0, import_firestore.getFirestore)(clientApp, firebase_applet_config_default.firestoreDatabaseId) : (0, import_firestore.getFirestore)(clientApp);
    } catch (err) {
...
app.post("/api/cron/refresh-trending", async (req, res) => {
  try {
    ...
    const hits = geminiHits.map((hit) => ({
      ...
      const daysOld = (Date.now() - relDateMs) / (1e3 * 60 * 60 * 24);
      return daysOld <= 90;
    });
    const colRef = (0, import_firestore.collection)(clientDb, "trending_hits");
    const existingDocs = await (0, import_firestore.getDocs)(colRef);
    const deleteBatch = (0, import_firestore.writeBatch)(clientDb);
    existingDocs.forEach((d) => deleteBatch.delete(d.ref));
    await deleteBatch.commit();
    const addBatch = (0, import_firestore.writeBatch)(clientDb);
    for (const hit of hits) {
      const docRef = (0, import_firestore.doc)(clientDb, "trending_hits", hit.id);
...
function decodeHtmlEntities(text2) {
  if (!text2) return "";
  return text2.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&apos;/g, "'").replace(/&#(\d+);/g, (_, code) => String.fromCharCode(parseInt(code, 10))).replace(/&#x([0-9a-fA-F]+);/g, (_, code) => String.fromCharCode(parseInt(code, 16)));
}
function cleanYouTubeTitle(rawTitle) {
  let clean = decodeHtmlEntities(rawTitle);
  clean = clean.replace(/\s*\(Official Video\)/gi, "").replace(/\s*\[Official Video\]/gi, "").replace(/\s*\(Official Music Video\)/gi, "").replace(/\s*\[Official Music Video\]/gi, "").replace(/\s*\(Lyric Video\)/gi, "").replace(/\s*\[Lyric Video\]/gi, "").replace(/\s*\(Official Audio\)/gi, "").replace(/\s*\[Official Audio\]/gi, "").replace(/\s*\(Audio\)/gi, "").replace(/\s*\[Audio\]/gi, "").replace(/\s*\|.*$/g, "").trim();
  if (clean.includes(" - ")) {
...
function isSingleSong(rawTitle, durationSeconds) {
    ...
    if (audiodbRes.ok) {
      ...
      if (data.artists && Array.isArray(data.artists) && data.artists.length > 0) {
        const matchedArtist = data.artists.find(
          (a) => a.strArtist && a.strArtist.toLowerCase() === artistName.toLowerCase()
        ) || data.artists[0];
...
function decryptSaavnMediaUrl(encUrl) {
  try {
    const key = Buffer.from("383465913834659138346591");
    const decipher = import_crypto.default.createDecipheriv("des-ede3", key, Buffer.alloc(0));
...
function cleanMatchingText(str) {
  return (str || "").toLowerCase().replace(/\(from.*?\)|\[.*?\]|\(official.*?\)|official music video|official video|video song|lyrical video|full video song|hd song|full song/gi, "").replace(/[^\w\s]/gi, " ").replace(/\s+/g, " ").trim();
}
function isValidSaavnMatch(targetTitle, targetArtist, match) {
  ...
  const duration = match.more_info?.duration ? parseInt(match.more_info.duration, 10) : match.duration ? parseInt(match.duration, 10) : void 0;
  if (duration !== void 0 && (duration < 50 || duration > 650)) {
    return false;
  }
  return true;
}
async function resolveFromJioSaavn(query, targetTitle, targetArtist) {
  ...
  try {
    ...
    for (const match of data.results) {
      ...
      if (enc) {
        ...
        if (fullUrl) {
          const duration = match.duration ? parseInt(match.duration, 10) : match.more_info?.duration ? parseInt(match.more_info.duration, 10) : void 0;
          const rawImage = match.image || match.more_info?.image;
          const coverUrl = rawImage ? rawImage.replace(/150x150|50x50/, "500x500") : void 0;
          return {
            url: fullUrl,
            coverUrl,
            duration: duration && duration > 0 ? duration : void 0,
            source: "saavn"
          };
        }
      }
...
async function scrapeYouTubeVideos(query, limit = 15) {
  try {
        ...
        if (Array.isArray(itemContents)) {
          for (const item of itemContents) {
            if (item.videoRenderer) {
              ...
              let title = vr.title?.runs?.map((r) => r.text).join("") || vr.title?.simpleText || "Unknown Title";
              let artist = vr.ownerText?.runs?.map((r) => r.text).join("") || vr.ownerText?.simpleText || "Unknown Artist";
              ...
              const cleanedTitle = title.replace(/\[(Official\s*(Video|Audio|Music Video|Lyric Video|4K|HD|Track)).*?\]/gi, "").replace(/\((Official\s*(Video|Audio|Music Video|Lyric Video|4K|HD|Track)).*?\)/gi, "").replace(/\|\s*(Full\s*(Video|Audio)\s*Song|Official\s*Music\s*Video|4K|Lyrical|Trending).*$/gi, "").trim();
...
function isAuthenticYouTubeVideoId(id) {
...
async function resolveYouTubeVideoBySong(title, artist, excludeVideoIds = []) {
        ...
        if (Array.isArray(itemContents)) {
          for (const item of itemContents) {
            if (item.videoRenderer) {
              ...
              const vTitle = vr.title?.runs?.map((r) => r.text).join("") || vr.title?.simpleText || "";
...
async function resolveAudioStreamInfo(videoId, title, artist, query, excludeVideoIds = []) {
  ...
  if (activeVideoId && activeVideoId.length === 11) {
    const audioUrl2 = await resolveYouTubeAudioUrl(activeVideoId);
    return {
      url: audioUrl2,
  ...
  if (activeTitle && activeArtist) {
    ...
    if (saavnStream && saavnStream.url) {
      streamCache.set(cacheKey, { url: saavnStream.url, videoId: activeVideoId, expiresAt: Date.now() + 6 * 3600 * 1e3 });
      return saavnStream;
    }
  }
...
app.get("/api/youtube/search", async (req, res) => {
  try {
    ...
    const maxResults = Math.min(parseInt(req.query.maxResults || "15", 10), 25);
    if (!query) {
    ...
    if (youtubeConfig.isConfigured()) {
      try {
        ...
        const searchRes = await fetch(searchUrl, { signal: AbortSignal.timeout(4e3) });
        if (searchRes.ok) {
          ...
          if (items.length > 0) {
            ...
            if (videoIds) {
              try {
                ...
                const detailsRes = await fetch(detailsUrl, { signal: AbortSignal.timeout(3e3) });
                if (detailsRes.ok) {
            ...
            const tracks = items.map((item) => {
              ...
              const coverUrl = item.snippet?.thumbnails?.high?.url || item.snippet?.thumbnails?.medium?.url || item.snippet?.thumbnails?.default?.url || "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600";
              let trackLang = "english";
    ...
    const filtered = MODULAR_CATALOG.filter((track) => {
      ...
      const allTokensMatch = searchTokens.length > 0 && searchTokens.every(
        (token) => titleL.includes(token) || artistL.includes(token) || movieL.includes(token) || albumL.includes(token)
      );
...
app.get("/api/youtube/recommendations", async (req, res) => {
  try {
    ...
    const language = (req.query.language || "all").toLowerCase();
    const excludeIdsRaw = (req.query.excludeIds || "").trim();
    const excludeIds = new Set(
      excludeIdsRaw ? excludeIdsRaw.split(",").map((s) => s.trim().replace(/^yt-/, "").replace(/^yt:/, "")).filter(Boolean) : []
    );
    const count = Math.min(parseInt(req.query.count || "15", 10), 30);
    const queries = [];
    if (language === "tamil") {
...
app.get("/api/youtube/playlist", async (req, res) => {
  try {
    ...
    const requestedMax = parseInt(req.query.maxResults || "50", 10);
    ...
    const videoIds = allItems.map((i) => i.contentDetails?.videoId).filter(Boolean).slice(0, 100).join(",");
    let durationsMap = {};
    if (videoIds) {
      try {
        const detailsUrl = youtubeConfig.buildApiUrl("videos", {
...
function cleanQueryForSearch(str) {
  if (!str) return "";
  return str.replace(/VEVO\b|- Topic\b|Official Channel\b/gi, "").replace(/\(.*?\)|\[.*?\]/g, "").replace(/\b(official|video|audio|lyric|lyrical|hd|4k|full song|movie|soundtrack|remix|single|feat|ft|music video|visualizer|audio song)\b/gi, "").replace(/[^\w\s]/gi, " ").replace(/\s+/g, " ").trim();
}
      ...
      if (cleanedQuery !== rawQuery) {
        ...
        if (rawRes.ok) {
          ...
          if (rawData.results && rawData.results.length > 0) {
            const item2 = rawData.results.find((r) => r.artworkUrl100) || rawData.results[0];
            const rawArtwork2 = item2.artworkUrl100 || item2.artworkUrl60;
            const artwork2 = rawArtwork2 ? rawArtwork2.replace(/100x100bb|60x60bb/, "1000x1000bb") : null;
            return res.json({
              title: item2.trackName,
              artist: item2.artistName,
              album: item2.collectionName,
              coverUrl: artwork2,
              coverArtUrl: artwork2,
              releaseDate: item2.releaseDate,
              genre: item2.primaryGenreName,
              previewUrl: item2.previewUrl,
              trackViewUrl: item2.trackViewUrl
            });
          }
        }
      }
...
app.get("/api/music/musicbrainz-metadata", async (req, res) => {
  try {
    ...
    const mbQuery = artist ? `recording:"${title}" AND artist:"${artist}"` : `recording:"${title}"`;
        ...
        try {
          ...
          if (caaRes.ok) {
            ...
            if (caaData.images && caaData.images.length > 0) {
              const frontImg = caaData.images.find((img) => img.front) || caaData.images[0];
...
app.get(["/api/youtube/stream", "/api/youtube/mp3"], async (req, res) => {
  try {
    const videoId = (req.query.id || "").replace("yt-", "").trim();
    const title = (req.query.title || "").trim();
    const artist = (req.query.artist || "").trim();
    if (!videoId && !title) {
    ...
    if (streamInfo && streamInfo.url) {
      if (streamInfo.url.startsWith("http")) {
        ...
        const proxyReq = import_https2.default.request(streamInfo.url, options, (proxyRes) => {
...
app.get("/api/stream/resolve", async (req, res) => {
  try {
    ...
    const artist = (req.query.artist || "").trim();
    const query = (req.query.q || "").trim();
    const excludeParam = (req.query.excludeId || "").trim();
...
var suggestionsCache = /* @__PURE__ */ new Map();
function backfillSuggestions(query, currentSuggestions) {
  const result = [...currentSuggestions];
  if (result.length >= 5) return result;
  const qLower = query.toLowerCase().trim();
  if (!qLower) return result;
  const wordsSet = /* @__PURE__ */ new Set();
  for (const t of MODULAR_CATALOG) {
    ...
    const parts = `${t.title} ${t.artist} ${t.movie || ""} ${t.album || ""}`.split(/[\s(),\-:._+]+/g).map((w) => w.trim()).filter((w) => w.length > 2);
...
app.get("/api/search/suggestions", async (req, res) => {
  try {
    ...
    try {
      ...
      if (suggestRes.ok) {
        ...
        if (Array.isArray(suggestData) && Array.isArray(suggestData[1])) {
          ...
          querySuggestions = suggestData[1].map((s) => String(s).trim()).filter((s) => {
            if (!s) return false;
    ...
    const cleanTokens = qLower.replace(/\b(songs?|singing|music|mp3|video|audio|track|lyrics?|full|official|hd|4k|status|bgm|remix|lofi|movie|film|original|single)\b/gi, "").split(/\s+/).filter((w) => w.length > 0);
    const matchingTracks = MODULAR_CATALOG.filter((t) => {
      ...
      if (tTitle.includes(qLower) || tArtist.includes(qLower) || tAlbum.includes(qLower) || tMovie.includes(qLower)) {
        return true;
      }
      if (cleanTokens.length > 0) {
        const matchTokens = cleanTokens.every(
          (tok) => tTitle.includes(tok) || tArtist.includes(tok) || tAlbum.includes(tok) || tMovie.includes(tok) || t.language.toLowerCase().includes(tok)
        );
...
app.get("/api/musicbrainz/search", async (req, res) => {
  try {
    ...
    const itunesTracks = itunesResults.map((it) => {
      const title = it.trackName || cleanQ;
      const artist = it.artistName || "Unknown Artist";
      const album = it.collectionName || "Single";
      const lengthMs = it.trackTimeMillis || 18e4;
      const duration = Math.round(lengthMs / 1e3);
      ...
      return {
        ...
        audioUrl: "",
        // resolved to YouTube stream when user taps song
        language,
        year: it.releaseDate ? parseInt(it.releaseDate.substring(0, 4), 10) : void 0,
        source: "musicbrainz"
      };
    });
    const mbTracks = recordings.map((r) => {
      const title = r.title || "Unknown Title";
      const artist = (r["artist-credit"] || []).map((a) => typeof a === "string" ? a : a.name || a.artist?.name || "").join("").trim() || "Unknown Artist";
      const release = (r.releases || [])[0];
      const album = release?.title || "Single";
      const releaseId = release?.id;
      const lengthMs = r.length || 18e4;
      const duration = Math.round(lengthMs / 1e3);
      const mins = Math.floor(duration / 60);
      const secs = duration % 60;
      const durationFormatted = `${mins}:${secs < 10 ? "0" : ""}${secs}`;
      const year = release?.date ? parseInt(release.date.substring(0, 4), 10) || void 0 : void 0;
      const coverUrl = releaseId ? `https://coverartarchive.org/release/${releaseId}/front-250.jpg` : "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600";
      let language = "english";
      const checkText = `${title} ${artist} ${album}`.toLowerCase();
      if (checkText.includes("tamil") || /[\u0B80-\u0BFF]/.test(checkText)) {
        language = "tamil";
...
app.get("/api/youtube/resolve-track", async (req, res) => {
  try {
    const title = (req.query.title || "").trim();
    const artist = (req.query.artist || "").trim();
    const query = (req.query.query || `${title} ${artist}`).trim();
    const excludeParam = (req.query.excludeId || "").trim();
    const excludeVideoIds = excludeParam ? excludeParam.split(",").map((s) => s.trim()).filter(Boolean) : [];
    if (!query && !title) {
    ...
    if (youtubeConfig.isConfigured()) {
      try {
        ...
        const ytRes = await fetch(searchUrl, { signal: AbortSignal.timeout(4e3) });
        if (ytRes.ok) {
